/*******************************************************************************
  ...
  Summary:
    Calls the different Stack Error generating routines.

  Description:
    The main function calls two different functions that generate a Stack Error
    trap such as the Stack Overflow or the Stack Underflow based on which one of 
    them is selected.
*******************************************************************************/
/*******************************************************************************
...
// Prototype declaration of imported functions from create_stkerr.s file
void CreateStkUflow ( void );
void    CreateStkOflow( void );
...
#endif
{
 ...
 test_flag=0;
    ...
    #if ( STACK_OVER_FLOW )
    CreateStkOflow();   // Create Stack overflow
    #endif
    #if ( STACK_UNDER_FLOW )
    CreateStkUflow();   // Create Stack underflow
    #endif
#ifdef TEST_MODE
     if (test_flag == 1)
