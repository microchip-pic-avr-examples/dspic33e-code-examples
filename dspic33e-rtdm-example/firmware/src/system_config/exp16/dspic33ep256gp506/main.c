/*******************************************************************************
  ...
  Description:
    ...
    and is used to call the RTDM_Start, RTDM_ProcessMsgs and RTDM_Close
    functions.
*******************************************************************************/
/*******************************************************************************
...
int main( void )
{
    ...
    RPINR18 = 0;
    RPOR1 = 0;
    RPOR1bits.RP37R = 3;                        //RP37/RB5 as U2TX
    RPINR19bits.U2RXR = 38;                     //RP38/RB6 as U2RX
    ANSELA = 0x0000;
    ANSELB = 0x0000;
    ANSELC = 0x0000;

    TRISAbits.TRISA8 = 0;                       // Config LED's
    TRISBbits.TRISB4 = 0;                       // Config LED's
    TRISCbits.TRISC0 = 0;                       // Config LED's
    TRISCbits.TRISC1 = 0;                       // Config LED's
    myFlags.TrigggerSnapshot = 1;               //Snapshot trigger intial state is OFF
    ...
    for( ;; )
    {
        LATCbits.LATC0 = ~myFlags.LED_D3;   //Passing data from the boolean variables controlled
        LATCbits.LATC1 = myFlags.LED_D4;    // in DMCI to the PORT A data registers
        LATBbits.LATB4 = ~myFlags.LED_D5;
        LATAbits.LATA8 = myFlags.LED_D6;
