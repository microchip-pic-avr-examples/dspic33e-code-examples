...
void InitPorts( void )
{
    ...
    // S6 (RG1,RD7,portE Pin 5, chosen as trigger for sending 'C' to UART)
    ...
    TRISD = 0x20C0;         // D6,D7,D13 inputs
    TRISAbits.TRISA7 = 1;   // F7 is input
...
int main( void )
{
    InitClock();    // This is the PLL InitPortssettings
    InitUART1();    // Initialize UART1 for 9600,8,N,1 TX/RX
