/*******************************************************************************
  ...
  Summary:
    Contains ADC and DMA configuration function definitions.

  Description:
    This source file configures the ADC to alternatively sample two analog input pins in 10-bit
    mode and starts conversion on every time out of the Timer3 module. The Timer3 is configured to 
    time out every 125 us. The DMA is configured in the ping pong mode thus accepting the results
    from the two conversions and placing them in two different buffer locations.    
 *******************************************************************************/
/*******************************************************************************
...
#ifdef _HAS_DMA_
__eds__ int16_t BufferA[MAX_CHNUM + 1][SAMP_BUFF_SIZE] __attribute__( (eds, space(dma), aligned(64)) );
__eds__ int16_t BufferB[MAX_CHNUM + 1][SAMP_BUFF_SIZE] __attribute__( (eds, space(dma), aligned(64)) );
#else
int16_t         BufferA[MAX_CHNUM + 1][SAMP_BUFF_SIZE] __attribute__( (space(xmemory), aligned(64)) );
int16_t         BufferB[MAX_CHNUM + 1][SAMP_BUFF_SIZE] __attribute__( (space(xmemory), aligned(64)) );
#endif
 ...
 * Side Effects:    None
 *
 * Overview:        This function is used to configure A/D to convert 4 inputs alternately and store the
 *                  conversion  results using the DMA.
 ...
 * Side Effects:    None
 *
 * Overview:        Direction: Read from peripheral address 0-x300 (ADC1BUF0) and write to DMA RAM
                    AMODE: Register indirect with post increment
                    MODE: Continuous, Ping-Pong Mode
                    IRQ: ADC Interrupt
                    ADC stores results stored alternatively between DMA_BASE[0]/DMA_BASE[16]
 *                  on every 16th DMA request
 *****************************************************************************/
void InitDma0( void )
{
...
#ifdef TEST_MODE
    unsigned char test_dma0_buf_flag=0;
    unsigned char test_dma1_buf_flag=0;
#endif
/******************************************************************************
