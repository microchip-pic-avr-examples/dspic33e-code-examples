 ...
 * Side Effects:    None
 *
 * Overview:       This function is a unlocksequence to write to write protected register
 *****************************************************************************/
...
void RtccInit( void )
{
    ...
    _RTCCIF = 0;                //clear the RTCC interrupt flag
    _RTCCIE = 1;                //enable the RTCC interrupt
}
...
void __attribute__ ( (interrupt) ) _RTCCInterrupt( void )
{
    ...
    _RTCCIF = 0;            // clear the INT1 interrupt flag or else the CPU will
